export default  function ($q,
  ...
  const IS_EMPTY_AFTER_SELECTION = true;

  // ---
  // Timeout.
  // ---
  const TIMEOUT_PENDING = 1000;
  ...
  // ---
  FileUploader.FileSelect.prototype.isEmptyAfterSelection = () => IS_EMPTY_AFTER_SELECTION;
   ...
   */
  $scope.isEnoughSelectedMatchingCategories = () => {
   ...
   */
  $scope.submitPerformImport = expensesImportForm => {
    if (expensesImportForm.$valid && !$scope.isImporting) {
      ...
      // Detach from scope.
      // ---
      const expensesImportPrepared = angular.copy($scope.expensesImportAnswer);

      // ---
      // We need to perform a transform of the selected categories.
      // ---
      _.each(
        expensesImportPrepared.model.expenseCategoryMatchingProfileDTOs,
        expenseCategoryMatchingProfileDTO => {
